"""``hope channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from hope.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "hope channels imessage-start <contact>",
        )
...
def imessage_start(
    ...
    if background:
        ...
        from hope.connectors.retriever import (
            TwoStageRetriever,
        )
        from hope.connectors.store import KnowledgeStore
        from hope.engine.ollama import OllamaEngine
        from hope.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from hope.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from hope.channels.imessage_daemon import stop_daemon
