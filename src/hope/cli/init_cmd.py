"""``hope init`` — detect hardware, generate config, write to disk."""
...
# Engines supported by ``hope init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     hope ask "Hello"\n'
            "\n"
            "  Run `hope doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    """Placeholder — the privacy scanner module was removed during voice-arch cleanup."""
    console.print(
        "  [dim]Privacy scanner removed; run any external audit you trust.[/dim]"
    )
    console.print()
...
def init(
    ...
    """Detect hardware and generate ~/.hope/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "hope" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]hope doctor[/bold] to verify."
        )
        return
...
"""
        ...
        console.print(
            ...
            "Run [bold]hope connect gdrive[/bold] to connect "
            "Google services, then [bold]hope digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Hope, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        if not no_download:
            console.print(
                "  [dim]Automatic model downloads were removed during the "
                "voice-arch cleanup — pull manually through your engine "
                "(e.g. `ollama pull {model}`).[/dim]".format(model=model)
            )
