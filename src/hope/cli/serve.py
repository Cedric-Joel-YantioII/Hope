"""``hope serve`` — OpenAI-compatible API server."""
...
from hope.engine import (
    discover_engines,
    discover_models,
    get_engine,
)
from hope.intelligence import (
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from hope.engine.cloud import CloudEngine
            from hope.engine.multi import MultiEngine
    ...
    try:
        from hope.telemetry.instrumented_engine import InstrumentedEngine
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import hope.tools  # noqa: F401  # trigger registration
    ...
    if config.agent.context_from_memory:
        try:
            import hope.tools.storage  # noqa: F401
    ...
    api_key = _os.environ.get("HOPE_API_KEY", "")
    if not api_key:
        try:
            import tomllib

            _cfg_path = str(
                __import__("pathlib").Path.home() / ".hope" / "config.toml"
            )
    ...
    from hope.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from hope.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting Hope API server[/green]\n"
