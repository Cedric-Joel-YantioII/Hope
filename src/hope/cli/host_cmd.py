"""``hope host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      hope host mlx-community/Qwen2.5-7B-4bit --backend mlx
      hope host Qwen/Qwen3-8B --backend vllm
      hope host qwen3:8b --backend ollama
      hope host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Hope will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
