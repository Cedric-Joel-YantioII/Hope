"""``hope config`` — configuration inspection commands."""
...
def _get_config_path(path: str | None) -> Path:
    ...
    from hope.core.config import DEFAULT_CONFIG_PATH

    if path:
        return Path(path)
    return Path(os.environ.get("HOPE_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from hope.core.config import detect_hardware, recommend_engine
...
def set_config(key: str, value: str) -> None:
    """Set a configuration value (e.g. hope config set engine.ollama.host URL)."""
    import tomlkit

    from hope.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("HOPE_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
