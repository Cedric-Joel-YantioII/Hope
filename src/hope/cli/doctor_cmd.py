"""``hope doctor`` — run diagnostic checks on the Hope installation."""
...
from hope.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `hope init` to generate a config file.",
    )
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "hope[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "hope[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "hope[energy-amd]", "AMD energy monitoring"),
        ("colbert", "hope[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "hope[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Hope installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Hope Doctor[/bold]")
    console.print()
