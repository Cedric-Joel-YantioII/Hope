"""``hope start|stop|restart|status`` — daemon management commands."""
...
def start(
    ...
    """Start the Hope server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'hope stop' to stop it first, or 'hope restart'.")
        sys.exit(1)
    ...
    # Build command to run hope serve
    cmd = [sys.executable, "-m", "hope.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Hope server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Hope server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Hope server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Hope server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
