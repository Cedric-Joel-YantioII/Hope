"""``hope agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: hope agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a HopeSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a HopeSystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from hope.agents.manager import AgentManager as _AM
