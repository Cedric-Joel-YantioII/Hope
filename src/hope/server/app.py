"""FastAPI application factory for the Hope API server."""
...
from hope.server.api_routes import include_all_routes
from hope.server.comparison import comparison_router
from hope.server.connectors_router import create_connectors_router
from hope.server.dashboard import dashboard_router
from hope.server.digest_routes import create_digest_router
from hope.server.routes import router
from hope.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from hope.server.channel_bridge import ChannelBridge
                    from hope.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional HopeConfig for other settings.
    """
    app = FastAPI(
        title="Hope API",
        description="OpenAI-compatible API server for Hope",
        version="0.1.0",
    )
    ...
    try:
        from hope.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from hope.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from hope.server.webhook_routes import (
