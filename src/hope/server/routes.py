...
from hope.server.models import (
    ChatCompletionChunk,
        ...
        try:
            ...
            if query_text:
                ...
                if len(enriched) > len(messages):
                    from hope.server.models import ChatMessage
            ...
            logging.getLogger("hope.server").debug(
...
def _handle_direct(
    ...
    if bus:
        from hope.telemetry.wrapper import instrumented_generate
...
def _handle_agent(
    ...
    if audio_path:
        ...
        from hope.server.models import AudioMeta
...
async def _handle_agent_stream(agent, bus, model, req):
    ...
    from hope.server.stream_bridge import create_agent_stream
...
async def _handle_stream(
    ...
    from hope.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("hope.server").error(
                "Stream error: %s",
                exc,
                exc_info=True,
            )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from hope.server.cloud_router import is_cloud_model, list_local_models
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.hope/cloud-keys.env and update the running process env.
    keys_path = Path.home() / ".hope" / "cloud-keys.env"
    if keys_path.exists():
...
async def security_scan():
    ...
    from hope.cli.scan_cmd import PrivacyScanner
