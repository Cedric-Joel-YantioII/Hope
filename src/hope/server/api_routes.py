...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import hope.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from hope.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from hope.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from hope.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from hope.tools.agent_tools import AgentSendTool
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from hope.tools.storage.ingest import ingest_path
...
async def list_skills(request: Request):
    ...
    try:
        from hope.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.hope/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from hope.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP hope_requests_total Total requests processed",
            "# TYPE hope_requests_total counter",
            f"hope_requests_total {stats.get('total_requests', 0)}",
            "# HELP hope_tokens_total Total tokens generated",
            "# TYPE hope_tokens_total counter",
            f"hope_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP hope_latency_avg_ms Average latency in milliseconds",
            "# TYPE hope_latency_avg_ms gauge",
            f"hope_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def include_all_routes(app) -> None:
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from hope.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from hope.server.ws_bridge import create_ws_router
