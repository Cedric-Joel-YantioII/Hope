"""API key authentication middleware for the Hope server."""
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, api_key: str = "") -> None:  # noqa: ANN001
        super().__init__(app)
        self._api_key = api_key or os.environ.get("HOPE_API_KEY", "")
...
def check_bind_safety(host: str, *, api_key: str) -> None:
    ...
    *api_key* is empty.
    ...
    if not is_loop and not api_key:
        logger.error(
            "Binding to %s requires HOPE_API_KEY to be set. "
            "Run: hope auth generate-key",
            host,
        )
        sys.exit(1)
