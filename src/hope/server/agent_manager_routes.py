...
logger = logging.getLogger("hope.server.agent_manager")
...
class _LightweightSystem:
    ...
    full HopeSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    """
    try:
        from hope.engine.ollama import OllamaEngine

        cfg = config
        if cfg is None:
            from hope.core.config import load_config
        ...
        try:
            from hope.core.events import get_event_bus
            from hope.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from hope.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import hope.channels  # noqa: F401
    except Exception:
        pass

    try:
        import hope.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in hope.tools.__init__)
    for _browser_mod in ("hope.tools.browser", "hope.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("hope.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("hope.tools.")
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from hope.core.credentials import TOOL_CREDENTIALS
    from hope.core.registry import ChannelRegistry, ToolRegistry
...
def _build_deep_research_tools(
    ...
    from hope.connectors.retriever import TwoStageRetriever
    from hope.connectors.store import KnowledgeStore
    from hope.tools.knowledge_search import KnowledgeSearchTool
    from hope.tools.knowledge_sql import KnowledgeSQLTool
    from hope.tools.scan_chunks import ScanChunksTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from hope.mcp.transport import StdioTransport, StreamableHTTPTransport
    from hope.tools.mcp_adapter import MCPToolProvider
    ...
    if agent_type == "deep_research" and app_state is not None:
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from hope.agents.deep_research import DeepResearchAgent
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from hope.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from hope.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from hope.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from hope.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from hope.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from hope.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from hope.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from hope.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from hope.channels.slack_daemon import (
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.hope/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from hope.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from hope.core.credentials import get_credential_status
