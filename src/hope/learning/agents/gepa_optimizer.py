...
Uses GEPA's adapter pattern to bridge Hope traces into GEPA's
...
from hope.core.config import GEPAOptimizerConfig
from hope.core.registry import LearningRegistry
from hope.learning._stubs import AgentLearningPolicy
...
class HopeGEPAAdapter:
    """Implements GEPA's adapter protocol for Hope agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'hope[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = HopeGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "HopeGEPAAdapter"]
