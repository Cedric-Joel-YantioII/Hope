"""Single point of contact between Python and the Rust ``hope_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``hope_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import hope_rust  # type: ignore[import-untyped]
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from hope.security.injection_scanner import (
        InjectionScanResult,
    )
    from hope.security.types import ScanFinding, ThreatLevel
