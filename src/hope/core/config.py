...
User configuration lives at ``~/.hope/config.toml``.  ``load_config()``
...
DEFAULT_CONFIG_DIR = Path.home() / ".hope"
DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"


def _ensure_config_dir() -> Path:
    """Ensure the config directory exists with restrictive permissions."""
    from hope.security.file_utils import secure_mkdir
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.hope/learning/skills/"
...
class StorageConfig:
    ...
    # Embedding strategy: "sync" = block during store() (default, backward-
    # compatible), "async" = enqueue and embed on a background thread so
    # store() returns immediately.
    embed_mode: str = "sync"
    # Preferred embedder identifier: "minilm" (default, backward-compat),
    # "bge-small-en-v1.5" (higher quality English, 384-dim), or "ollama".
    embedder: str = "minilm"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through Hope. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.hope/whatsapp_auth
    assistant_name: str = "Hope"
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.hope/operators"
    auto_activate: str = ""  # Comma-separated operator IDs


@dataclass(slots=True)
class VisionConfig:
    """On-device vision (multimodal) settings — Gemma 4 E4B via MLX.

    The model is *not* loaded at startup. The first ``VISION_REQUEST`` event
    triggers a lazy load; after ``idle_timeout_sec`` of inactivity the model
    is unloaded to reclaim RAM (~2.5-3 GB on Apple Silicon).
    """

    enabled: bool = True
    model_repo: str = "mlx-community/gemma-4-e4b-it-4bit"
    ...
    # Prefer mlx-vlm (native multimodal). Fallback path stays off unless forced.
    backend: str = "mlx_vlm"  # "mlx_vlm" | "mlx_lm"
...
class SpeechConfig:
    ...
    # Always-on STT (whisper.cpp / distil-large-v3.5)
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.hope/SOUL.md"
    memory_path: str = "~/.hope/MEMORY.md"
    user_path: str = "~/.hope/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.hope/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/hope/skill-index.git"
...
# Sections that users may set via ``hope config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(HopeConfig.__dataclass_fields__.keys()) - {"hardware"}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``HopeConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import hope.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "HopeConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> HopeConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``HOPE_CONFIG`` when set,
        otherwise ``~/.hope/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = HopeConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("HOPE_CONFIG"):
        config_path = Path(os.environ["HOPE_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``hope init``)
# ---------------------------------------------------------------------------
...
# Full reference config: hope init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.hope/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.hope/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.hope/whatsapp_auth
# assistant_name = "Hope"
