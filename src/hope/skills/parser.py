...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.hope.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "hope_meta" the value is
# stored under manifest.metadata["hope"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("hope_meta", "platforms"),
    "prerequisites": ("hope_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.hope.original_frontmatter,
        and merges metadata.hope.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize hope namespace
        oj_meta = dict(raw_metadata.get("hope") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.hope.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.hope.* into canonical fields (these override
        # top-level mappings since they are explicit Hope-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.hope.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the hope metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["hope"] = oj_meta
