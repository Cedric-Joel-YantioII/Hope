...
from hope.skills.dependency import validate_dependencies
from hope.skills.executor import SkillExecutor, SkillResult
from hope.skills.loader import discover_skills
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.hope/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.hope/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.hope.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from hope.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("hope", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["hope"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.hope.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("hope", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.hope/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.hope/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "hope (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.hope/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from hope.skills.loader import load_skill_directory
