...
Recipes are the universal composition format for Hope.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".hope" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".hope" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Hope.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Hope constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from hope.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from hope.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.hope/recipes/`` directory
    4. User ``~/.hope/operators/`` directory
