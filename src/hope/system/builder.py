"""Config-driven fluent builder that wires up a HopeSystem."""
...
from hope.core.config import HopeConfig, load_config
from hope.core.events import EventBus, get_event_bus
from hope.engine._stubs import InferenceEngine
from hope.system.core import HopeSystem
from hope.tools._stubs import BaseTool, ToolExecutor

logger = logging.getLogger(__name__)


class SystemBuilder:
    """Config-driven fluent builder for HopeSystem."""
    ...
    def build(self) -> HopeSystem:
        """Construct a fully wired HopeSystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from hope.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from hope.telemetry.gpu_monitor import GpuMonitor
        ...
        from hope.security import setup_security
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.hope/agents.db").expanduser()
                )
        ...
        if speech_enabled:
            try:
                from hope.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: HopeConfig):
        from hope.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: HopeConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import hope.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import hope.channels  # noqa: F401 -- trigger registration
            from hope.core.registry import ChannelRegistry
            from hope.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from hope.sandbox.runner import ContainerRunner
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: HopeConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from hope.core.config import DEFAULT_CONFIG_DIR
            from hope.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from hope.learning.training.lora import LoRATrainingConfig
            from hope.traces.store import TraceStore
