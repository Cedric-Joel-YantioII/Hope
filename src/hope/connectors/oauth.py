...
class OAuthProvider:
    ...
    # Filenames in ~/.hope/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.hope/connectors/`` and falls
    back to environment variables ``HOPE_{NAME}_CLIENT_ID`` and
    ``HOPE_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"HOPE_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Hope.</p>"
                    b"</body></html>"
                )
