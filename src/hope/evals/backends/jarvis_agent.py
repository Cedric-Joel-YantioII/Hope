"""Hope Agent backend — agent-level inference with tool calling."""
...
class HopeAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + HopeSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "hope-agent"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. HopeConfig.agent.max_turns
