...
from hope.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a HopeSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after HopeSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from hope.agents import AgentRegistry
        ...
        # Resolve engine + model from HopeSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in HopeSystem")
        ...
        if router_policy_key and self._system:
            try:
                from hope.core.registry import RouterPolicyRegistry
                from hope.learning.routing.types import (
