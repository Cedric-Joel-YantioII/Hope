...
from hope.agents.digest_store import DigestArtifact, DigestStore
from hope.core.registry import AgentRegistry
from hope.core.types import Message, Role, ToolCall


def _load_persona(persona_name: str) -> str:
    """Load a persona prompt file by name."""
    search_paths = [
        Path("configs/hope/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".hope" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._persona = kwargs.pop("persona", "hope")
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from hope.agents.digest_evaluator import DigestEvaluator
