...
from hope.engine._base import InferenceEngine
...
def _make_engine(key: str, config: HopeConfig) -> InferenceEngine:
...
def discover_engines(config: HopeConfig) -> List[Tuple[str, InferenceEngine]]:
...
def get_engine(
    config: HopeConfig, engine_key: str | None = None
