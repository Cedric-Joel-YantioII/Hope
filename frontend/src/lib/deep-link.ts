/**
 * deep-link.ts — Handler for hope:// deep links in the Tauri desktop app.
 *
 ...
 *   hope://research/{session_id}   → open a research session
 *   hope://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `hope://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `hope://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `hope:` scheme.
 *
 * @example
 * parseDeepLink("hope://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("hope://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "hope:") return null;

    // The URL constructor treats "hope://research/abc123" such that
