"""Tests for intent-based agent routing in HopeSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal HopeSystem instance for testing _detect_agent_intent."""
        from hope.system import HopeSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = HopeSystem.__new__(HopeSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("hope.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_hope_triggers_digest(self, system):
        with patch("hope.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Hope")
