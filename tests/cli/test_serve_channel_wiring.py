"""Tests for HopeSystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on HopeSystem directly. The serve.py
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> HopeSystem:
    """Build a minimal HopeSystem with mock engine for testing wire_channel."""
    config = HopeConfig()
...
# ---------------------------------------------------------------------------
# Tests via HopeSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """HopeSystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """HopeSystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """HopeSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = HopeConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
