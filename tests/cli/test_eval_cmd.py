"""Tests for the ``hope eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``hope eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``hope eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "hope-direct" in result.output
        assert "hope-agent" in result.output

    def test_eval_run_missing_args(self):
        """``hope eval run`` without required args fails gracefully."""
