"""Tests for ``hope init`` next-steps guidance."""
...
from hope.cli.init_cmd import _next_steps_text
...
class TestInitShowsNextSteps:
    def test_init_shows_next_steps(self, tmp_path: Path) -> None:
        ...
        assert "hope ask" in result.output
        assert "hope doctor" in result.output
...
class TestNextStepsOllama:
    def test_next_steps_ollama(self) -> None:
        ...
        assert "hope ask" in text
        assert "hope doctor" in text
...
class TestMinimalConfig:
    def test_init_generates_minimal_by_default(self, tmp_path: Path) -> None:
        """Default hope init produces a short config."""
        config_dir = tmp_path / ".hope"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
        ...
        assert "hope init --full" in content

    def test_init_full_generates_verbose_config(self, tmp_path: Path) -> None:
        """hope init --full produces the full reference config."""
        config_dir = tmp_path / ".hope"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("hope.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("hope.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("hope.cli.init_cmd.PrivacyScanner") as MockScanner,
        ):
            from hope.cli.scan_cmd import ScanResult
            ...
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "hope scan" in result.output
