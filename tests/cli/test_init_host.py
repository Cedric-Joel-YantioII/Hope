"""Tests for ``hope init --host`` option."""
...
from hope.core.config import generate_default_toml, generate_minimal_toml
...
class TestInitHost:
    def test_init_host_writes_to_config(self, tmp_path: Path) -> None:
        """hope init --host writes the host into config.toml."""
    ...
    def test_init_host_with_vllm(self, tmp_path: Path) -> None:
        """hope init --host applies to the selected engine."""
        config_dir = tmp_path / ".hope"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
    ...
    def test_init_host_probes_and_reports(self, tmp_path: Path) -> None:
        """hope init --host shows reachability status."""
        config_dir = tmp_path / ".hope"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("hope.cli.init_cmd.PrivacyScanner"),
            mock.patch("hope.cli.init_cmd.httpx") as mock_httpx,
        ):
    ...
    def test_init_without_host_still_works(self, tmp_path: Path) -> None:
        """hope init without --host still produces valid config."""
        config_dir = tmp_path / ".hope"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("hope.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestGenerateTomlHost:
    def test_minimal_toml_with_host(self) -> None:
        from hope.core.config import HardwareInfo
