...
from hope.core.types import Message, Role, TelemetryRecord
from hope.telemetry.aggregator import AggregatedStats, TelemetryAggregator
from hope.telemetry.instrumented_engine import InstrumentedEngine
from hope.telemetry.store import TelemetryStore

_ask_mod = importlib.import_module("hope.cli.ask")
_bench_mod = importlib.import_module("hope.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a HopeConfig with energy monitoring enabled."""
    cfg = HopeConfig()
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "hope.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Hope instance."""
        from hope.sdk import Hope
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "hope.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "hope.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "hope.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "hope.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
