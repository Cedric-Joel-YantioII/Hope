"""Verify that importing hope.tools registers all built-in tools."""
...
def _reload_tool_modules() -> None:
    """Reload all hope.tools.* submodules to re-trigger @register decorators.

    The autouse ``_clean_registries`` fixture clears all registries before each
    test.  A plain ``import hope.tools`` won't re-register because the
