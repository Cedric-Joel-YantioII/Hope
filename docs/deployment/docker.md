...
Hope provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.

## Quick Start

The fastest way to get Hope running in Docker is with Docker Compose, which starts both the API server and an Ollama backend:
...
1. **Builder stage** -- installs `uv` and the `hope[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker run -d -p 8000:8000 hope:latest
```
...
```bash
docker build -f Dockerfile.gpu -t hope:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 hope:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the Hope API server and an Ollama backend:
...
services:
  hope:
    ...
    ports:
      - "8000:8000"
    environment:
      - HOPE_ENGINE_DEFAULT=ollama
      - HOPE_OLLAMA_HOST=http://ollama:11434
    depends_on:
      - ollama
...
The `hope` service is configured through environment variables:

| Variable                      | Description                                             | Default                    |
|-------------------------------|---------------------------------------------------------|----------------------------|
| `HOPE_ENGINE_DEFAULT`   | Inference engine backend to use                         | `ollama`                   |
| `HOPE_OLLAMA_HOST`      | URL of the Ollama server (uses Docker service name)     | `http://ollama:11434`      |
...
The `hope` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.hope/config.toml`, which is `/root/.hope/config.toml` in the container):
...
services:
  hope:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.hope/config.toml:ro
    environment:
      - HOPE_ENGINE_DEFAULT=ollama
      - HOPE_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire Hope data directory:
...
The entrypoint is `hope` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 hope:latest \
...
### Available CLI Options for `hope serve`
