...
```bash
uv run hope --version   # Should print 0.1.0
uv run hope --help      # Show all subcommands
```
...
Hope uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=hope --cov-report=html
```
...
Hope uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/hope/`:
...
src/hope/
    ...
    core/                       # Shared infrastructure
        config.py               # HopeConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # hope ask
        init_cmd.py             # hope init
        model.py                # hope model list/info
        memory_cmd.py           # hope memory index/search/stats
        telemetry_cmd.py        # hope telemetry stats/export/clear
        bench_cmd.py            # hope bench run
        serve.py                # hope serve
```
...
try:
    import hope.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
...
- Absolute imports only (`from hope.core.registry import ...`)
