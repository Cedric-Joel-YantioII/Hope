...
- **Persistence:** Data persists across restarts in `~/.hope/memory.db`
...
!!! note "Backward compatibility"
    The old imports (e.g., `from hope.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `hope.tools.storage.*`.
...
```python
from hope.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from hope.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from hope.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.hope/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
