...
```python
from hope.agents.orchestrator import OrchestratorAgent
...
!!! note "Backward compatibility"
    The old `from hope.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.
...
```python
from hope.agents.native_openhands import NativeOpenHandsAgent
...
```python
from hope.agents.rlm import RLMAgent
...
```python
from hope.agents.openhands import OpenHandsAgent
...
Wraps the `@anthropic-ai/claude-code` SDK via a bundled Node.js subprocess bridge. Unlike every other agent, inference is handled entirely by the Claude Agent SDK -- the Hope inference engine is not used. This makes `ClaudeCodeAgent` a true external agent, similar in spirit to `OpenHandsAgent` but implemented via subprocess rather than an importable Python SDK.
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.hope/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
4. Reads stdout and extracts the JSON payload between `---HOPE_OUTPUT_START---` and `---HOPE_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `hope-sandbox=true`
...
from hope.agents._stubs import BaseAgent
