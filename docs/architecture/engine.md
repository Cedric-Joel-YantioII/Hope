...
The Engine primitive provides the **inference runtime** -- the layer that connects Hope to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
Hope automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from hope.engine import discover_engines
...
```python
from hope.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.hope/config.toml` using **nested per-engine sub-sections**:
...
```python
from hope.engine._base import messages_to_dicts
...
```python
from hope.engine import EngineConnectionError
