...
Hope follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Hope is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Hope is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Hope(engine_key="ollama")  # Local Ollama server
...
Hope **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `hope init` and used as the default engine:

```bash
hope init --force
# Detects hardware, writes ~/.hope/config.toml with:
# [engine]
...
```bash
hope telemetry stats          # Aggregated statistics
hope telemetry export --json  # Export all records
```
...
Hope provides a **clean Python API** through the `Hope` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`hope[server]`, `hope[memory-colbert]`, etc.)
...
The API server (`hope serve`) implements the **OpenAI chat completions API format**, making Hope a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Hope:
...
Hope requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Hope.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `hope[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `hope[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `hope[memory-bm25]` | `rank-bm25` |
| API server | `hope[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `hope[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `hope[inference-vllm]` | `vllm` |
| PDF ingestion | `hope[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `hope[channel-whatsapp-baileys]` | Node.js 22+ |
