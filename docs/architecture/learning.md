...
!!! note "Backward compatibility"
    The canonical locations are now `hope.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `hope.core.types` (for `RoutingContext`). The old `hope.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `hope.learning._stubs`.
...
```bash
hope ask --router learned "What is the capital of France?"
```
...
```python
from hope.learning.router import HeuristicRouter, build_routing_context
...
```python
from hope.learning.router import build_routing_context
...
```python
from hope.learning.trace_policy import TraceDrivenPolicy
from hope.traces.analyzer import TraceAnalyzer
...
```python
from hope.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from hope.learning.sft_policy import SFTPolicy
```
...
```python
from hope.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from hope.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from hope.learning.heuristic_reward import HeuristicRewardFunction
...
store = TraceStore("~/.hope/traces.db")
...
over Hope's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `hope-learning`
