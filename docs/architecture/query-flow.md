...
This page traces the end-to-end journey of a user query through the Hope system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: hope ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: HopeConfig (hardware, engine defaults)
...
Hope supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
hope ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.hope/config.toml
```
...
- Returns a `HopeConfig` dataclass with all settings
...
```python
from hope.learning import ensure_registered
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.hope/telemetry.db`.
...
The `Hope` class in `sdk.py` provides the same query flow through a Python API:

```python
from hope import Hope

j = Hope(model="qwen3:8b", engine_key="ollama")
