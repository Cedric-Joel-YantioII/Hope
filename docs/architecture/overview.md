...
description: The five-primitive architecture behind Hope — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Hope is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Hope Architecture](../assets/Hope_Architecture.png)
...
The sandbox module (`hope.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Hope use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/hope/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           HopeConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              hope ask -- query the assistant
        serve.py            hope serve -- start API server

    sdk.py              Hope class -- high-level Python SDK
