---
title: Code Snippets
description: Copy-paste patterns for common Hope tasks
---

# Code Snippets

Ready-to-use patterns for the most common Hope tasks. Each snippet is self-contained and copy-pasteable.
...
Run with: `hope compose run research_assistant "quantum computing advances"`

## API Server (1 command)

```bash
hope serve --port 8000 --engine ollama --model qwen3:8b
```
...
```bash
docker build -t hope .
docker run -p 8000:8000 hope serve --host 0.0.0.0
```
