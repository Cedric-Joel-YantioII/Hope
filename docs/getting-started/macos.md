...
description: Complete step-by-step guide to installing Hope on macOS with llama.cpp, including common pitfalls and fixes
search:
  boost: 2
---
...
This guide walks through a complete Hope installation on macOS using **llama.cpp** as
...
| Git | Clones the Hope repo |
| Node.js | Required for the browser UI |
| Rust | Compiles the Hope security and memory extension |
| llama.cpp | Local inference engine that runs GGUF model files |
| Hope | The framework itself |
...
`uv` replaces pip, virtualenv, and pyenv in one tool. Hope uses it to manage Python
...
Git is used to clone the Hope source code. It may already be present if you have
...
Hope includes a Rust extension that provides security scanning, memory indexing,
...
!!! warning "Do not clone from inside an existing Hope folder"
    A common mistake is running `git clone` while already inside the repo, creating deeply
    nested duplicates (`Hope/Hope/Hope`). Always clone from `~` or a
...
!!! warning "Critical step — do not skip"
    Hope requires Python 3.10–3.13. Its Rust extension uses PyO3, which does not yet
    ...
    with `ModuleNotFoundError: No module named 'hope_rust'`.
...
```bash
uv run python -c "import hope_rust; print('Rust extension OK')"
```
...
Hope needs a GGUF model file to run inference. First install the Hugging Face CLI,
...
# Terminal 2 — backend
cd ~/Hope && uv run hope serve --port 8000

# Terminal 3 — frontend
cd ~/Hope/frontend && npm run dev
```
...
```bash
source ~/Hope/.venv/bin/activate
```

Your prompt will show `(hope)` when active, and you can type `hope ask "..."` directly.
...
2. Rebuild: `uv run maturin develop -m rust/crates/hope-python/Cargo.toml`
...
llama-server is not running. Start it in a separate terminal before running any `hope`
...
- [Architecture](../architecture/overview.md) — Understand how Hope is structured
