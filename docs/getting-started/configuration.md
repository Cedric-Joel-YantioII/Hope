---
title: Configuration
description: Complete reference for Hope configuration
---

# Configuration

Hope uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Hope creates the `~/.hope/` directory and populates it with a default config when you run `hope init`.
...
3. Writes `~/.hope/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `hope init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Hope automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Hope uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
hope ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.hope/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `hope serve`.
...
```bash
hope serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.hope/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.hope/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.hope/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.hope/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `hope init`, Hope probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.hope/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Hope entirely from Python without a TOML file:
...
```python
j = Hope(config_path="/path/to/my-config.toml")
```
...
Hope respects the following environment variables:
