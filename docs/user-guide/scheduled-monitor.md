...
This writes a pre-configured `~/.hope/config.toml` for the operative agent with scheduling support.
...
hope scheduler status <task-id>

# Pause / resume / delete a task
hope scheduler pause <task-id>
hope scheduler resume <task-id>
hope scheduler delete <task-id>

# Run a task immediately (outside its schedule)
hope scheduler run <task-id>
...
- **All data stays local**: State is stored in `~/.hope/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `hope scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `hope scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `hope memory stats`. Clear old entries with `hope memory clear --before 2026-01-01` if needed.
