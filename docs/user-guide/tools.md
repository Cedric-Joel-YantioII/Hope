...
# Execute a tool call
from hope.core.types import ToolCall
...
```python
from hope.tools._stubs import build_tool_descriptions
...
```python
from hope.tools.retrieval import RetrievalTool
from hope.memory.sqlite import SQLiteMemory
...
```python
from hope.tools.llm_tool import LLMTool
...
```python
from hope.tools.file_read import FileReadTool
...
```python
from hope.scheduler.tools import ScheduleTaskTool
from hope.scheduler.scheduler import TaskScheduler
from hope.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.hope/scheduler.db")
scheduler = TaskScheduler(store=store, system=hope_system)
scheduler.start()
...
```bash
hope ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
hope ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
hope ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
