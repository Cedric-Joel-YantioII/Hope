...
This writes a pre-configured `~/.hope/config.toml` for the deep research agent.
...
Hope chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
hope ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
hope ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
hope memory index ~/Downloads/reports/
hope memory index ./notes.md

# Search memory directly
hope memory search "project timeline"
hope memory search -k 20 "budget estimates"
...
hope ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
hope ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
hope ask "What meetings did I have with Alice last month?"

# Extract structured data
hope ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
hope ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
hope memory index ./report.pdf

# Custom chunk size for long documents
hope memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
curl -s https://example.com/article | hope memory index --stdin --source "example.com"
```

### Code repositories

```bash
hope memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `hope memory index`. Check indexed content with `hope memory stats`.
