...
- **Persistence:** SQLite database file (default: `~/.hope/memory.db`)
...
```python
from hope.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from hope.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `hope.tools.storage.*` imports.
...
[Source: docs/intro.md] Hope is a modular AI framework...
...
hope memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
hope memory search "machine learning"
