...
This writes a pre-configured `~/.hope/config.toml` for the code assistant.
...
```bash
hope ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
hope ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
hope ask "Read main.py and explain the architecture"

# Fix a bug
hope ask "Find and fix the bug in test_utils.py"

# Run tests
hope ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
hope ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.hope/config.toml`:
...
```bash
# Write a new script
hope ask "Write a Python script that converts YAML to JSON"

# Explain existing code
hope ask "Read src/hope/core/events.py and explain the EventBus pattern"

# Debug a failing test
hope ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
hope ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
hope ask "Read src/hope/tools/calculator.py and write unit tests for it"

# Shell tasks
hope ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`hope chat`).
- For stronger isolation, use the sandboxed agent: `hope ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `hope`. Use `cd /path && command` in your prompt if needed, or run `hope` from the project directory.
