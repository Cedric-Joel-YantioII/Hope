...
The Hope evaluation framework (`hope-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main Hope library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Hope has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `hope-evals` | Correctness on academic datasets (accuracy, pass rate) | `hope-eval` |
    | **Benchmarks** | `hope` | Engine performance (latency, throughput) | `hope bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside Hope:

```bash
uv sync --extra eval
```

This installs the `hope-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real Hope use cases.
...
```bash
uv run python -m hope.evals --config src/hope/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m hope.evals --config src/hope/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **hope-direct** | `hope-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **hope-agent** | `hope-agent` | Agent-level inference with tool calling. Uses `HopeSystem.ask()` with the specified agent and tools. |

Use `hope-direct` for most evaluations. Use `hope-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.
...
| `--backend` | | choice | `hope-direct` | `hope-direct` or `hope-agent` |
| `--model` | `-m` | str | required* | Model identifier (e.g., `qwen3:8b`, `gpt-4o`) |
| `--engine` | `-e` | str | auto | Engine key (`ollama`, `vllm`, `cloud`, ...) |
| `--agent` | | str | `orchestrator` | Agent name for `hope-agent` backend |
...
```bash
hope-eval run-all -m qwen3:8b

# With options
hope-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
hope-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
hope-eval run --config src/hope/evals/configs/full-suite.toml
```
...
| `backend` | str | `"hope-direct"` | Inference backend: `hope-direct` or `hope-agent` |
...
| `agent` | str | `None` | Agent name for `hope-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `hope-agent` backend |
...
hope-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `hope-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to Hope inference and agents
