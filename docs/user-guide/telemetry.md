...
Hope has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.hope/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.hope/telemetry.db")
...
hope telemetry stats -n 5          # Top 5 models only

# Export records
hope telemetry export              # JSON to stdout
hope telemetry export -f csv       # CSV to stdout
hope telemetry export -o data.json # JSON to file
hope telemetry export -f csv -o metrics.csv

# Clear all records
hope telemetry clear               # With confirmation prompt
hope telemetry clear --yes         # Without confirmation
```
