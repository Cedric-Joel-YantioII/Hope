...
Hope has two types of integrations:
...
```json
{
    ...
    "features": {
        ...
        "bot_user": { "display_name": "Hope", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @Hope in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Hope" |
...
3. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect gmail_imap`
...
Slack serves two purposes in Hope:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [Hope icon](https://github.com/open-hope/Hope/blob/main/assets/hope-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **Hope** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Hope"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Hope` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Hope` in the channel |
...
3. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect notion`
...
3. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Hope desktop app
...
3. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect outlook`
...
2. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect obsidian --path /path/to/vault`
...
4. **Connect in Hope:**
   ...
   - CLI: `uv run hope connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run hope deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.hope/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.hope/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
