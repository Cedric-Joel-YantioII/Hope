...
!!! note "Optional component"
    The scheduler is a standalone module (`hope.scheduler`). It is not wired into the default `Hope` / `HopeSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `hope scheduler start`.
...
The `hope scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
hope scheduler list --status active

# Paused tasks
hope scheduler list --status paused
```
...
hope scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
hope scheduler resume a3f9b12c4d8e
```
...
hope scheduler cancel a3f9b12c4d8e
```
...
hope scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.hope/scheduler.db")  # (1)!

# Wire in a HopeSystem for task execution
...
4. Starts a daemon thread named `"hope-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.hope/config.toml`.

```toml title="~/.hope/config.toml"
[scheduler]
enabled = false
db_path = "~/.hope/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.hope/scheduler.db`     | SQLite database path                       |
