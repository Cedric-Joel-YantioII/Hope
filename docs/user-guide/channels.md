...
The channels module lets Hope send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from hope.channels.telegram import TelegramChannel
...
from hope.channels._stubs import ChannelMessage
from hope.channels.discord_channel import DiscordChannel
...
from hope.channels.slack import SlackChannel
...
The `hope channel` subcommand group provides quick access to channel operations.
...
hope channel send telegram "Build completed successfully"
```
...
When `hope serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.hope/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.hope/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from Hope!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  Hope
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. Hope sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since Hope runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from hope.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from Hope!")
```
...
hope channel status --channel-type sendblue

# Send a message
hope channel send sendblue "+15551234567" "Hello from Hope!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `hope serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.hope/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from hope.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Hope",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.hope/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.hope/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Hope"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.hope/whatsapp_baileys_bridge/auth"
assistant_name = "Hope"
...
- [API Reference: Channels](../api-reference/hope/channels/index.md) -- full class and type signatures
