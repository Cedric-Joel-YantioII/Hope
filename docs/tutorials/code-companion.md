...
1. Both `model` and `engine_key` are optional. Omitting them uses auto-detected defaults from `~/.hope/config.toml`.
...
4. The tool list is passed directly. Any registered tool name is valid — run `hope agent info native_react` to see all available tools.
