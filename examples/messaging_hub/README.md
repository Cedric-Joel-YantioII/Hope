...
Hope orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting Hope to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Hope installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Hope channel docs).
...
Hope supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/hope/config.toml` for the full list of channel and agent
options.
...
this via the Hope scheduler:

```bash
hope scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/hope/recipes/data/operators/` for examples.
