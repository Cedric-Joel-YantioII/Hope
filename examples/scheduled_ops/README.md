...
This tutorial demonstrates how to use Hope to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Hope` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `hope scheduler` CLI
...
Register each script as a recurring task with `hope scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | hope channel send slack
...
See `hope channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Hope auto-select from what is available.
