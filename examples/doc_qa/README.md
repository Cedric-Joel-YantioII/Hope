...
Index a directory of documents into Hope memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- Hope installed (`git clone https://github.com/open-hope/Hope.git && cd Hope && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Hope.memory.index()` to chunk the documents at
...
Hope SDK. Adjust `--chunk-size` and `--top-k` to tune the
