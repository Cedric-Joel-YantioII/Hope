...
using Hope. The assistant uses an orchestrator agent loop with web
...
- Hope installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `hope eval`
runner.
...
User query
  |
  v
Hope SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `hope agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `hope.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/hope/recipes/) -- composable TOML configs that
