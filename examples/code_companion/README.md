...
A set of developer-focused scripts that use Hope tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Hope` instance with the `native_react` (ReAct) agent
...
1. **Install Hope** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/hope/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/hope/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run hope eval list` or
inspect `src/hope/tools/` for the full registry.
