# Hope PR Review Instructions

You are reviewing pull requests for Hope, a local-first personal AI agent framework built with Python, Rust (PyO3), and TypeScript.
...
- **Registry pattern compliance** — new components (engines, tools, agents, channels) must register via `ToolRegistry`, `EngineRegistry`, `AgentRegistry`, `ChannelRegistry`, etc. in `src/hope/core/registry.py`
- **Event bus integration** — new lifecycle events should use `EventBus` from `src/hope/core/events.py`
